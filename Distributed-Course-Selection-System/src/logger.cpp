...
void Logger::parseDelVal2(string cmd,string &key,string &val){
    int pos=cmd.find(",");
...
bool Logger::commitLog(string &ans){
    ...
    if(oper=="DEL"){
        ...
        parseDelVal2(log[posi-1].op.value,_key,_val);
        ...
        //     }
        // }
        if(db.delValue(_key,_val,oldval)!=0){
                cerr << "[ERROR] del wrong <logger.cpp 74>\n";
                return false;
            }else{
                log[posi-1].op.delVal.push_back(oldval);
    ...
    if(oper=="SET"){
        if(kv_debug3==1)printf("--------------------logger.96\n");
        string _key,_val,oldval;
        if(kv_debug3==1)cout << "[log 111] " << log[posi-1].op.value << endl;

        parseSetVal(log[posi-1].op.value,_key,_val);

        if(kv_debug3==1)cout << "sadfasdf: " << _key << ' ' << _val << endl;
        if(db.setValue(_key,_val,oldval)!=0){
            if(kv_debug3==1)printf("--------------------logger.107\n");
            return false;
        }else{
    ...
    if(kv_debug3==1)printf("--------------------logger.113\n");
    return true;
}
...
bool Logger::rollbackLog(){
    ...
    if(kv_debug3==1)printf("--------------------logger.127\n");
    if(log[_posi].state==LOG_PRE){
        log.pop_back();
    }else if(log[_posi].state==LOG_COMMIT){
        if(kv_debug3==1)printf("--------------------logger.131\n");
        if(op=="GET"){
            posi--;
        }else if(op=="DEL"){
            int delpos=0;
            ...
            while(!delqueue.empty()){
                ...
                delqueue.pop();
                string t;
                if(db.setValue(tkey,log[_posi].op.delVal[delpos++],t)!=0){
                    cerr << "[ERROR] recover del operation failed <logger.cpp 120>\n";
                    return false;
                }
            }
        }else if(op=="SET"){
            if(kv_debug3==1)printf("--------------------logger.146\n");
            int pos;
            ...
            string t;
            if(db.setValue(key,val,t)!=0){
                return false;
            }
        }
        log.pop_back();
    }
    if(kv_debug3==1)printf("--------------------logger.158\n");
    posi--;
    return true;
}
