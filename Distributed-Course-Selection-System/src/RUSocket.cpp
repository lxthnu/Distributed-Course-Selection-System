...
void* RUSocket::handle_while(void * arg){
    ...
    while(1){
        if(kv_debug4==1)printf("--------------------RUSs65\n");
        sem_wait(&ROsem[0]);
        pthread_mutex_lock(&mutex1);
        descT = tcp.getMessage();
        pthread_mutex_unlock(&mutex1);
        if(kv_debug4==1) printf("--------------------RUSs69\n");
        while(!descT.empty()) {
            ...
            if(!desc->enable_message_runtime)
            {
                ...
                message.push(desc);
                cout << desc->message << endl;
                if(kv_debug4==1)printf("--------------------RUSs77\n");
                sem_post(&ROsem[1]);
            }
        }
        if(kv_debug4==1)printf("--------------------RUSs83\n");
    }
