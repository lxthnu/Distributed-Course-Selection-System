...
public:
    ...
    void Start(TCPServer tcp,unordered_map<string, course> cmap,unordered_map<string, string> smap,string s1addr,string s2addr,int s1port,int s2port); //一切的开始
...
private:
    ...
    void handleListen_();
    void handleWrite_(HTTPconnection* client,TCPServer tcp);
    void handleRead_(HTTPconnection* client,TCPServer tcp);
